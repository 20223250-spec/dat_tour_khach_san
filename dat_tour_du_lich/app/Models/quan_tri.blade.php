...
<head>
    ...
    <title>@yield('title', 'Quản lý - TourBooking')</title>
...
@php
    ...
    $adminPortalUrl = fn (string $tab) => route('home', ['admin_active_tab' => $tab]) . '#quan-tri-noi-bo';
    $dashboardRoute = $currentUser?->isAdmin()
        ? $adminPortalUrl('tong-quan')
        : ($currentUser?->canManageTours()
            ? route('partner.tours.index')
            : route('partner.rooms.index'));
    $tourRoute = $currentUser?->isAdmin() ? $adminPortalUrl('tour-du-lich') : route('partner.tours.index');
    $roomRoute = $currentUser?->isAdmin() ? $adminPortalUrl('phong') : route('partner.rooms.index');
    $portalLabel = $currentUser?->isAdmin() ? 'Admin TourBooking' : 'Cổng đối tác TourBooking';
    $portalIcon = $currentUser?->isAdmin() ? 'fa-crown' : 'fa-briefcase';
@endphp

<nav class="admin-topbar navbar navbar-expand-lg navbar-dark sticky-top">
    <div class="container-fluid px-3 px-lg-4">
        <a href="{{ $dashboardRoute }}" class="topbar-brand">
            <i class="fa-solid {{ $portalIcon }} me-2"></i>{{ $portalLabel }}
        </a>
    ...
    <div class="row g-0">
        <aside class="col-12 col-lg-2 admin-sidebar">
            ...
            @if($currentUser?->isAdmin())
                <a class="side-link active" href="{{ $adminPortalUrl('tong-quan') }}" data-admin-target="tong-quan">
                    <i class="fa-solid fa-gauge"></i>
                    <span>Tổng quan</span>
                </a>
                <a class="side-link" href="{{ $adminPortalUrl('don-dat') }}" data-admin-target="don-dat">
                    <i class="fa-solid fa-receipt"></i>
                    <span>Đơn đặt</span>
                </a>
            @endif

            @if($currentUser?->canManageTours())
                <a class="side-link {{ ! $currentUser?->isAdmin() && request()->routeIs('partner.tours.*') ? 'active' : '' }}" href="{{ $tourRoute }}" @if($currentUser?->isAdmin()) data-admin-target="tour-du-lich" @endif>
            ...
            @if($currentUser?->canManageRooms())
                <a class="side-link {{ ! $currentUser?->isAdmin() && request()->routeIs('partner.rooms.*') ? 'active' : '' }}" href="{{ $roomRoute }}" @if($currentUser?->isAdmin()) data-admin-target="phong" @endif>
            ...
            @if($currentUser?->isAdmin())
                ...
                <a class="side-link" href="{{ $adminPortalUrl('bao-cao') }}" data-admin-target="bao-cao">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const adminLinks = Array.from(document.querySelectorAll('[data-admin-target]'));
        ...
        const syncAdminLinks = () => {
            const activeTarget = window.location.hash.replace('#', '') || 'tong-quan';

            adminLinks.forEach((link) => {
                link.classList.toggle('active', link.dataset.adminTarget === activeTarget);
            });
        };

        adminLinks.forEach((link) => {
            link.addEventListener('click', () => {
                window.setTimeout(syncAdminLinks, 0);
            });
        });

        window.addEventListener('hashchange', syncAdminLinks);
        syncAdminLinks();
    });
</script>
@stack('scripts')
