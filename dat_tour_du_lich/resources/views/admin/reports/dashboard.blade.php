...
@section('content')
    @php
        $revenueChartData = $revenueByMonth->map(function ($item) {
            return [
                'year' => $item->year,
                'month' => $item->month,
    ...
    <section class="panel-header">
        <div class="d-flex flex-column flex-lg-row justify-content-between gap-3">
            <div>
                <h1 class="h3 mb-2">Báo cáo tổng quan</h1>
                <p class="mb-0 text-white-50">Tổng hợp doanh thu, trạng thái đơn đặt và xu hướng vận hành gần đây.</p>
            </div>
                            ...
                            <div class="border rounded-4 p-3 h-100">
                                <div class="d-flex justify-content-between align-items-center gap-2 mb-1">
                                    <div class="fw-semibold">
                                        <span class="d-inline-block me-2 rounded-circle bg-primary status-dot status-dot-{{ $loop->index }}"></span>
                                        {{ $status['label'] }}
                                    </div>
                                    <div class="fw-semibold">{{ $status['percent'] }}%</div>
                                </div>
                                <small class="text-muted">{{ number_format($status['count'], 0, ',', '.') }} đơn</small>
                            </div>
                        </div>
                    @endforeach
                </div>
...
@push('scripts')
    ...
    <script>
        const statusColors = ['#f3c45f', '#25b6a5', '#ee7878', '#6b93ff'];
        document.querySelectorAll('.status-dot').forEach((dot, index) => {
            dot.style.width = '10px';
            dot.style.height = '10px';
            dot.style.backgroundColor = statusColors[index] || '#6b93ff';
        });
        ...
        new Chart(statusCtx, {
            ...
            data: {
                ...
                datasets: [{
                    data: [
                        <?= (int) $statusBreakdown['pending']['count'] ?>,
                        <?= (int) $statusBreakdown['confirmed']['count'] ?>,
                        <?= (int) $statusBreakdown['cancelled']['count'] ?>,
                        <?= (int) $statusBreakdown['completed']['count'] ?>
                    ],
        ...
        const revenueData = <?= json_encode($revenueChartData, JSON_UNESCAPED_UNICODE) ?>;
