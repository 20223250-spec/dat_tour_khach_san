...
@section('title', ($isAdminArea ? 'Đăng phòng mới' : 'Thêm phòng mới') . ' - TourBooking')

@section('content')
    <section class="panel-header">
        <h1 class="h3 mb-0">{{ $isAdminArea ? 'Tạo bài đăng phòng' : 'Đăng phòng mới' }}</h1>
    </section>
    ...
    <section class="panel-card p-4 p-lg-5">
        ...
        <form method="POST" action="{{ route($routePrefix . '.store') }}" enctype="multipart/form-data" class="row g-4">
            ...
            <div class="col-md-6">
                ...
                <input type="text" class="form-control" id="hotel_name" name="hotel_name" value="{{ old('hotel_name') }}" required>
            </div>
            ...
            @if($isAdminArea)
                <div class="col-md-6">
                    ...
                    <small class="text-muted d-block mt-2">Admin chỉ quản trị và phân công, không đứng tên phòng.</small>
                </div>
            @endif
            ...
            <div class="col-md-4">
                ...
                <input type="number" class="form-control" id="price_per_night" name="price_per_night" value="{{ old('price_per_night') }}" min="0" required>
            </div>

            <div class="col-md-4">
                <label for="guest_capacity" class="form-label fw-semibold">Sức chứa</label>
                <input type="number" class="form-control" id="guest_capacity" name="guest_capacity" value="{{ old('guest_capacity', 2) }}" min="1" required>
            </div>

            <div class="col-md-4">
                <label for="available_rooms" class="form-label fw-semibold">Số phòng trống</label>
                <input type="number" class="form-control" id="available_rooms" name="available_rooms" value="{{ old('available_rooms', 1) }}" min="0" required>
            </div>
            ...
            <div class="col-md-6">
                ...
                <select id="status" name="status" class="form-select" required>
                    ...
                    <option value="hidden" @selected(old('status') === 'hidden')>Ẩn tạm thời</option>
                </select>
            </div>
